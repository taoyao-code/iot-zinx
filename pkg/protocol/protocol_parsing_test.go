	...
	t.Run("真实DNY协议帧解析_永久标准", func(t *testing.T) {
		testCases := []struct {
			...
			},
			{
				...
				expectedCommand:    0x80,
				expectedMessageID:  0x0246,
...
func TestChecksumCalculation_Permanent(t *testing.T) {
	t.Run("校验和计算算法_永久标准", func(t *testing.T) {
		testCases := []struct {
			...
			{
				name:             "真实DNY帧校验和2",
				hexData:          "444e595000f36ca20403001168020220fc58681f07383938363034443931363233393034383832393755000038363434353230363937363234373256312e302e30302e3030303030302e30363136000000000026", // 不包含校验和的部分
				expectedChecksum: 0x1126,
				reason:           "长帧的校验和计算",
			},
		}
