...
func NeedConfirmation(command uint8) bool {
	...
	noConfirmationCommands := []uint8{
		...
		// 🔧 查询类指令 - 服务器查询，设备直接应答，无需再次确认
		0x81,                         // 查询设备联网状态 - 协议标注：设备应答：无须应答
		0x90, 0x91, 0x92, 0x93, 0x94, // 查询参数指令 - 设备直接应答参数内容，无需再次确认
		0x8B, // 服务器读取EEPROM的数据 - 设备直接应答数据内容
		0x8C, // 服务器写入EEPROM的数据 - 设备直接应答执行结果

		// 🔧 心跳和状态上报指令 - 服务器应答后设备无需再次确认
		0x01, 0x21, // 设备心跳包 - 协议标注：服务器应答后设备无需再次确认
		...
		0x43, // 充电完成通知但不结算 - 协议标注：服务器应答：无需应答
		0x44, // 端口推送指令 - 协议标注：服务器应答：无须应答

		// 🔧 设备主动请求指令 - 服务器直接处理或应答，设备无需再次确认
		0x05, // 设备主动请求升级 - 协议标注：服务器应答：无须应答
		0x09, // 分机测试模式 - 协议标注：服务器无需处理，不会发送给服务器
		0x0A, // 分机设置主机模块地址 - 协议标注：服务器无需处理，不会发送给服务器

		// 🔧 设置类指令 - 服务器设置，设备应答执行结果，无需再次确认
		0x83, 0x84, 0x85, 0x86, // 设置运行参数指令 - 设备应答执行结果后无需再次确认
		0x87, 0x88, 0x89, // 复位重启、存储器清零、播放语音 - 设备应答执行结果后无需再次确认
		0x8D, 0x8E, 0x8F, // 设置工作模式、二维码地址、TC刷卡模式 - 设备应答执行结果后无需再次确认
		0x95, 0x96, 0x97, 0x98, // 临时二维码、声光寻找设备、运行参数1.3、多功能指令 - 设备应答执行结果后无需再次确认
		0x72, // 充电柜停止充电但不开柜门 - 设备应答执行结果后无需再次确认

		// 🔧 升级类指令 - 服务器发送升级包，设备应答当前状态，无需再次确认
		0xE0, 0xE1, 0xE2, 0xE4, // 设备固件升级指令 - 设备应答当前包状态后无需再次确认
		0xF8, // 老版本设备固件升级 - 设备应答当前包状态后无需再次确认
		0xFA, // 主机固件升级指令 - 协议明确："请求-应答"模式，服务器发送应答后主机无需再次回复确认
		0x15, // 主机请求固件升级（老版本）- 协议明确："请求-应答"模式，服务器发送应答后主机无需再次回复确认

		// 🔧 主机控制类指令 - 服务器发送控制命令，主机应答执行结果，无需再次确认
		0x31, // 重启主机指令 - 主机应答执行结果后无需再次确认
		0x32, // 重启通讯模块指令 - 主机应答执行结果后无需再次确认
		0x33, // 清空升级分机数据指令 - 主机应答执行结果后无需再次确认
		0x34, // 更改IP地址指令 - 主机应答执行结果后无需再次确认
		0x3A, // 设置FSK主机参数及分机号 - 主机应答执行结果后无需再次确认
