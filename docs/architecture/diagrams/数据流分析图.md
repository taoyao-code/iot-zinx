...
sequenceDiagram
    ...
    %% 5. 响应生成和发送阶段
    Note over Device,ResponseSender: 5. 响应生成和发送阶段
    BusinessLogic->>ResponseSender: 构造响应数据
    ResponseSender->>ResponseSender: BuildDNYPacket()
    Note right of ResponseSender: 构造DNY响应帧:<br/>设置PhysicalID<br/>设置MessageID<br/>设置Command<br/>添加Payload<br/>计算Checksum
    
    ResponseSender->>ZinxFramework: SendMsg()
    ZinxFramework->>TCPPort: 发送响应数据
    TCPPort->>Device: TCP响应数据包
