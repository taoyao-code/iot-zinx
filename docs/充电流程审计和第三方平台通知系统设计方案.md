...
type SessionData struct {
    ...
    PortNumber   int       `redis:"port_number"`
    OrderID      string    `redis:"order_id"`
    UserID       string    `redis:"user_id"`
    CardNumber   string    `redis:"card_number"`
    StartTime    int64     `redis:"start_time"`
    EndTime      int64     `redis:"end_time"`
    Duration     int       `redis:"duration"`
    TotalEnergy  float64   `redis:"total_energy"`
    MaxPower     int       `redis:"max_power"`
    AvgPower     int       `redis:"avg_power"`
    TotalFee     int       `redis:"total_fee"`
    StartReason  string    `redis:"start_reason"`
    StopReason   int       `redis:"stop_reason"`
    FinalStatus  int       `redis:"final_status"`
    CreatedAt    int64     `redis:"created_at"`
    UpdatedAt    int64     `redis:"updated_at"`
}

// 设备活跃会话映射：String结构
// Key: device:{deviceID}:port:{portNumber}:session
// Value: sessionID
// TTL: 24小时

// 设备会话历史：List结构
// Key: device:{deviceID}:sessions
// Value: sessionID列表（最近100个）
// TTL: 30天
```
...
```go
// 事件数据：List结构
// Key: events:{deviceID}:{date}  // 按设备和日期分组
// Value: JSON格式的事件数据
// TTL: 7天
type AuditEventData struct {
    ...
    ProtocolCommand byte                   `json:"protocol_command"`
...
```go
// 通知记录：Hash结构
// Key: notification:{eventID}:{endpoint}
// TTL: 3天
type NotificationLog struct {
    EventID         string            `redis:"event_id"`
    EndpointName    string            `redis:"endpoint_name"`
    URL             string            `redis:"url"`
    Method          string            `redis:"method"`
    RequestHeaders  string            `redis:"request_headers"`  // JSON字符串
    RequestBody     string            `redis:"request_body"`     // JSON字符串
    ResponseStatus  int               `redis:"response_status"`
    ResponseHeaders string            `redis:"response_headers"` // JSON字符串
    ResponseBody    string            `redis:"response_body"`
    ResponseTimeMs  int               `redis:"response_time_ms"`
    AttemptCount    int               `redis:"attempt_count"`
    Success         bool              `redis:"success"`
    ErrorMessage    string            `redis:"error_message"`
    CreatedAt       int64             `redis:"created_at"`
}

// 通知统计：Hash结构
// Key: notification:stats:{endpoint}:{date}:{hour}
// TTL: 7天
type NotificationStats struct {
    EndpointName      string `redis:"endpoint_name"`
    Date              string `redis:"date"`
    Hour              int    `redis:"hour"`
    TotalSent         int    `redis:"total_sent"`
    TotalSuccess      int    `redis:"total_success"`
    TotalFailed       int    `redis:"total_failed"`
    TotalRetried      int    `redis:"total_retried"`
    AvgResponseTimeMs int    `redis:"avg_response_time_ms"`
    UpdatedAt         int64  `redis:"updated_at"`
}
```
...
```go
// Redis存储操作示例
type RedisAuditStorage struct {
    client redis.Client
    config *StorageConfig
}

// 保存充电会话
func (r *RedisAuditStorage) SaveSession(session *ChargingSession) error {
    key := fmt.Sprintf("session:%s", session.SessionID)

    // 转换为Redis Hash
    fields := map[string]interface{}{
        "device_id":     session.DeviceID,
        "port_number":   session.PortNumber,
    ...
    pipe.Expire(key, r.config.TTL.Sessions)

    // 设备会话映射
    deviceKey := fmt.Sprintf("device:%s:port:%d:session", session.DeviceID, session.PortNumber)
    pipe.Set(deviceKey, session.SessionID, 24*time.Hour)
...
// 保存审计事件
func (r *RedisAuditStorage) SaveEvent(event *AuditEvent) error {
    date := time.Unix(event.Timestamp.Unix(), 0).Format("2006-01-02")
    key := fmt.Sprintf("events:%s:%s", event.DeviceID, date)

    eventData, _ := json.Marshal(event)

    pipe := r.client.Pipeline()
    pipe.LPush(key, eventData)
    pipe.Expire(key, r.config.TTL.Events)

    // 限制列表长度
    pipe.LTrim(key, 0, 999) // 保留最近1000个事件
